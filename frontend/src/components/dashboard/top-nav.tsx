...
import { Shield, Bell, ChevronDown, Play } from 'lucide-react';
...
export function TopNav({ onOpenHITL }: TopNavProps) {
  return (
    <header className="shrink-0 flex items-center justify-between px-2 py-0.5 2xl:py-1">
      ...
      {/* Right controls */}
      <div className="flex items-center gap-2.5 2xl:gap-3">
        {/* Quick Launch Scan / HITL Review button */}
        <button
          onClick={onOpenHITL}
          className="flex items-center gap-1.5 rounded-lg bg-brand px-3 py-1.5 font-mono text-xs font-semibold text-white shadow-sm hover:bg-brand/90 transition-all hover:shadow"
        >
